...
async def _execute_booking(params: FunctionCallParams, doctor_id: str, patient_name: str, start_time_iso: str, phone: str, reason: str, force_book: bool = False, is_followup: bool = False, is_same_patient: str = "unknown", existing_patient_id: str = None):
    ...
    try:
        ...
        if is_followup:
            ...
            # Trigger 'the_free_followup_confirmation_telugu' template
            # Vars: {{1}} Clinic, {{2}} Name, {{3}} Time, {{4}} Date, {{5}} Token
            template_vars = [
                "Mithra Hospitals", # {{1}}
                clean_name,         # {{2}}
                appt_time_str,      # {{3}}
            ...
            await send_whatsapp_template(
                ...
                template_name="the_free_followup_confirmation_telugu", # 👈 MATCHES META
        ...
        if payment_link:
            # Extract just the dynamic ID from the end of the Razorpay URL (e.g., '03g2TZTK')
            rzp_short_id = payment_link.split("/")[-1]
            
            # Trigger 'the_payment_link_message_telugu' template
            # Vars: {{1}} Clinic, {{2}} Name, {{3}} Time, {{4}} Date, {{5}} Amount, Button: ID
            template_vars = [
                "Mithra Hospitals",     # {{1}}
                clean_name,             # {{2}}
                appt_time_str,          # {{3}}
                appt_date_str,          # {{4}}
                str(consultation_fee)   # {{5}}
            ]
            
            await send_whatsapp_template(
                phone_number=clean_phone,
                template_name="the_payment_link_message_telugu", # 👈 MATCHES META
                language_code="en",                              # 👈 META EXPECTS ENGLISH
                body_variables=template_vars,
