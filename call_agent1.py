...
from pipecat.services.sarvam import SarvamTTSService  # 👈 Swapped Google for Sarvam
...
class AutoLanguageProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TextFrame):
            ...
            target_locale = "en-IN" # Sarvam uses en-IN instead of en-US
            if has_telugu_chars:
                target_locale = "te-IN"
            elif has_hindi_chars:
                target_locale = "hi-IN"

            # 👈 Sarvam v3 female voices (e.g. priya, ritu, neha, shruti)
            female_voices = {
                "te-IN": "priya",
                "hi-IN": "priya",
                "en-IN": "priya"
            ...
            if target_locale != self.active_locale:
                ...
                # 👈 Use Sarvam Settings wrapper
                await self.push_frame(
                    TTSUpdateSettingsFrame(
                        settings=SarvamTTSService.Settings(
                            language=target_locale,
                            voice=target_voice_id,
                            pace=1.0 if target_locale == "te-IN" else 0.95
                        )
                    ),
                    direction
                )
...
async def run_bot(room_name: str, session_call_uuid: str = "livekit_call", inbound_caller_id: str = "Unknown"):
    ...
    # 👈 Switched to SarvamTTSService (bulbul:v3)
