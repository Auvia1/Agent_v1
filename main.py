...
@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("⚙️ Initializing Cloud Database and Redis...")
    pool = await get_db_pool()
    init_tool_db(pool)
    
    # Initialize Redis for both agent files
...
# Attach the endpoints from your agent files to the main app!
app.include_router(whatsapp_agent.router)
app.include_router(call_agent.router)

# ==========================================================
# 💳 GLOBAL RAZORPAY WEBHOOK (Kept in main to avoid duplication)
# ==========================================================
@app.post("/razorpay-webhook")
async def razorpay_webhook(request: Request):
...
# ==========================================================
# 🚀 AUTO-LAUNCHER (Ngrok, Redis, and Server)
# ==========================================================
if __name__ == "__main__":
    try:
        subprocess.Popen(["redis-server"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        logger.warning(f"⚠️ Could not start Redis locally: {e}")

    try:
        subprocess.Popen(["ngrok", "http", "8000"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        logger.info("✅ Ngrok Tunnel started on port 8000.")
    except Exception as e:
        logger.warning(f"⚠️ Could not start Ngrok: {e}")

    time.sleep(2)
    logger.info("🚀 Launching Unified Mithra Call & WhatsApp Server...")
    uvicorn.run("main:app", host="0.0.0.0", port=8000, reload=True)